...
function sfsi_plus_social_buttons_below( $content, $inline = true, $sfsi_section8 = null, $gutenberg = false ) {
	...
	if (
		...
		if ($inline) {
			$icons = "<div class='sfsi_plus_Sicons_style_2 sfsi_plus_Sicons " . $float . "' style='" . $style . $style_parent . "'>
	...
	if ( $sfsi_section8['sfsi_plus_recttwtr'] == 'yes' ) {
		...
		$icons .= "<div class='sf_twiter' style='display: inline-flex;vertical-align: middle;width: auto;'><a href='https://twitter.com/intent/tweet?text=" . urlencode($twitter_text) . "' " . sfsi_plus_checkNewWindow() . " style='display:inline-block' ><img nopin=nopin class='sfsi_premium_wicon' src='" . $tweet_icon . "' alt='".__( 'Tweet', 'ultimate-social-media-plus' )."' title='".__( 'Tweet', 'ultimate-social-media-plus' )."' ></a>" . $count_html . "</div>";
	...
	if ($sfsi_section8['sfsi_plus_rectlinkedin'] == 'yes') {
		...
		$icons .= "<div class='sf_linkedin' style='display: inline-flex;vertical-align: top;text-align:left;width: " . $sfsiLikeWithlinkedin . "'><a href='https://www.linkedin.com/shareArticle?url=" . $permalink . "'><img class='sfsi_premium_wicon' nopin=nopin alt='".__( 'Share', 'ultimate-social-media-plus' )."' title='".__( 'Share', 'ultimate-social-media-plus' )."' src='" . $linkedin_share_icon . "'  /></a>" . $count_html . "</div>";
	...
	if ($sfsi_section8['sfsi_plus_rectshr'] == 'yes') {
		$icons .= "<div class='sf_addthis' style='display: inline-flex;vertical-align: middle;width: auto;'>" . sfsi_plus_Addthis($show_count, $permalink, $title) . "</div>";
	}
	$icons .= "</div>";
...
function sfsi_premium_social_responsive_buttons($content, $option8, $server_side = false, $option5 = null) {
	...
	if ((isset($option8["sfsi_plus_show_item_onposts"]) && $option8["sfsi_plus_show_item_onposts"] == "yes" && isset($option8["sfsi_plus_display_button_type"]) && $option8["sfsi_plus_display_button_type"] == "responsive_button") || $server_side) :
				...
				foreach ($sfsi_premium_responsive_icons['default_icons'] as $icon => $icon_config) {
					...
					if (false == $is_pinterest || (true == $is_pinterest && $icon_config['url'] !== "")) {
						if (isset($option5['sfsi_plus_url_shorting_api_type_setting']) && $option5['sfsi_plus_url_shorting_api_type_setting'] == "bitly") {
							$share_url = untrailingslashit($share_url);
						}
		...
		function sfsi_plus_pinitpinterest($permalink, $show_count)
		{
			...
			$pinit_html = '<a href="https://www.pinterest.com/pin/create/button/?url=' . $permalink . '&media=' . $socialObj->sfsi_pinit_image() . '&description=' . str_replace("+", "%20", str_replace('#', '%23', urlencode($socialObj->sfsi_pinit_description()))) . '" data-pin-do="buttonPin" data-pin-save="true"';
			if ($show_count) {
		...
		function sfsi_redditShareButton( $url = '' ) {
			...
			return $ifollow = '<a href="//www.reddit.com/submit?url=' . $url . '" onclick="' . $onclick . '"><img nopin=nopin src="'.SFSI_PLUS_PLUGURL.'images/reddit-share.jpg" alt="'.__( 'submit to reddit', 'ultimate-social-media-plus' ).'" style="border:0" /></a>';
		}