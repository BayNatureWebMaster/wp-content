...
function sfsi_plus_options_updater5()
{
    ...
    $sfsi_plus_facebook_icons_language   =  isset($_POST["sfsi_plus_facebook_icons_language"])
        ...
        : 'Visit_us_en_US';
    $sfsi_plus_youtube_icons_language   =  isset($_POST["sfsi_plus_youtube_icons_language"])
        ? sanitize_text_field($_POST["sfsi_plus_youtube_icons_language"])
        : 'Visit_us_en_US';
    $sfsi_plus_twitter_icons_language    =  isset($_POST["sfsi_plus_twitter_icons_language"])
        ...
        : 'Visit_us_en_US';
    $sfsi_plus_linkedin_icons_language     =  isset($_POST["sfsi_plus_linkedin_icons_language"])
        ? sanitize_text_field($_POST["sfsi_plus_linkedin_icons_language"])
        : 'en_US';
...
function sfsi_plus_options_updater8()
{
    ...
    $sfsi_plus_rectshr                  = isset($_POST["sfsi_plus_rectshr"]) ? sanitize_text_field($_POST["sfsi_plus_rectshr"]) : 'no';
...
function sfsi_premium_responsive_settings_echoed()
{
    $option8 = maybe_unserialize(get_option('sfsi_premium_section8_options', false));
    $return_data = array();
    $return_data["device"] = array("desktop" => $option8["sfsi_plus_responsive_icons_show_on_desktop"], "mobile" => $option8["sfsi_plus_responsive_icons_show_on_mobile"]);
...
function sfsi_premium_get_feed_id()
{
    ...
    if (!current_user_can('manage_options')) {
        echo json_encode( array( 'res' => 'Failed', 'message' => __( 'You should be admin to take this action', 'ultimate-social-media-plus' ) ) );
        exit;
    }
...
function sfsi_premium_recheck_license()
{
    ...
    if ($license_api_name == 'ultimate') {
        ...
        $response = wp_remote_post(SELLCODES_API_URL . "/check_license", array('timeout' => 30, 'sslverify' => false, 'body' => $api_params));
        if (!is_wp_error($response) || 200 === wp_remote_retrieve_response_code($response)) {
