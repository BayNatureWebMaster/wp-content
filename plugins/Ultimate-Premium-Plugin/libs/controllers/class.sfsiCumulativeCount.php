...
class sfsiCumulativeCount
{
   ...
   public function __construct($urlHttp=false,$urlHttps=false,$access_token='') {
    ...
    public function sfsi_get_multi_requests($data, $options = array(),$outputTypeJson=false) {
      ...
      $responses = Requests::request_multiple($curly);
    ...
    public function sfsi_get_multi_curl($data, $options = array(),$outputTypeJson=false) {
        if(method_exists('Requests','request_multiple')){
          $requests = array();
          foreach ($data as $id => $d) {
            $single_req = array();
    ...
    private function sfsi_get_api_url($apiType,$urlArr=false){
        ...
        $apiUrl    = 'https://graph.facebook.com/v11.0/?ids='.$arrJson.'&fields=og_object{engagement}&access_token='.$this->access_token;
    ...
    public function sfsi_count_cumulative($arrFbData = false){
        ...
        if(!empty($arrFbData)){
            ...
            if( isset( $httpData->og_object->engagement->count ) || isset( $httpsData->og_object->engagement->count ) ):
                
                $httpsCount = isset( $httpsData->og_object->engagement->count) ? $httpsData->og_object->engagement->count : 0;
                $httpCount = isset( $httpData->og_object->engagement->count) ? $httpData->og_object->engagement->count : 0;

                if( isset( $httpDataWSlash ) && false !== $httpDataWSlash ) {
                  $httpDataWSlashCount = isset( $httpDataWSlash->og_object->engagement->count ) ? $httpDataWSlash->og_object->engagement->count : 0;
                }

                if( isset( $httpsDataWSlash ) && false !== $httpsDataWSlash ) {
                  $httspDataWSlashCount = isset( $httpsDataWSlash->og_object->engagement->count ) ? $httpsDataWSlash->og_object->engagement->count : 0;
                }

                $count = ( isset( $httpCount ) ? $httpCount : 0 ) + ( isset( $httpsCount ) ? $httpsCount : 0 ) + ( isset( $httpDataWSlashCount ) ? $httpDataWSlashCount : 0 ) + ( isset( $httspDataWSlashCount ) ? $httspDataWSlashCount : 0 );
            endif;
        }
        return $count;
    }
