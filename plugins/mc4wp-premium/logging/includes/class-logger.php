...
class MC4WP_Logger
{
    ...
    public function add_hooks()
    {
        ...
        add_action('mctb_subscribed', [ $this, 'log_top_bar_request' ], 10, 4);
        add_action('mc4wp_integration_subscribed', [ $this, 'log_integration_request' ], 10, 5);
        add_action('mc4wp_form_subscribed', [ $this, 'log_form_request' ], 10, 4);
        add_action('mc4wp_logging_purge_old_items', [ $this, 'purge_old_items' ]);

        add_filter('wp_privacy_personal_data_exporters', [ $this, 'register_exporter' ], 90);
        add_filter('wp_privacy_personal_data_erasers', [ $this, 'register_eraser' ], 90);
    }
     ...
     * @return false|int
     */
    public function log_integration_request(MC4WP_Integration $integration, $email, $merge_vars, $data_map = [], $related_object_id = 0)
    {
     ...
     * @return false|int
     */
    public function log_form_request(MC4WP_Form $form, $email_address, array $merge_vars, array $data_map = [])
    {
        $data = [
            'type' => 'mc4wp-form',
    ...
    protected function hydrate($results)
    {
        ...
        foreach ($results as $raw_result) {
            ...
            foreach ($raw_result as $property => $value) {
                if (in_array($property, [ 'success' ])) {
                    continue;
                }

                if (in_array($property, [ 'merge_fields', 'interests']) && is_string($value)) {
    ...
    {
        $log_items = $this->find([ 'email' => $email_address, 'limit' => 10000 ]);
        $data_to_export = [];
