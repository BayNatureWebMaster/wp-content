...
abstract class Tribe__Events__Community__Captcha__Abstract_Captcha {
	...
	public function init() {
		...
		add_filter( 'tec_events_community_allowed_fields', [ $this, 'whitelist_form_fields' ], 10, 1 );
	...
	public function validate_submission( $valid, $submission, $submission_handler ) {
		if ( $this->showing_captcha() ) {
			...
			if ( ! $valid_captcha ) {
				...
				$messages->add_message( __( 'Invalid value for the Anti-Spam Check', 'tribe-events-community' ), 'error' );
				$valid = false;
			}
		}
		return $valid;
