...
class MC4WP_Ecommerce
{
    ...
    * @const string
    */
    public const META_KEY = 'mc4wp_updated_at';
    ...
    */
    ...
    public const ERR_NO_ITEMS = 29001;
    public const ERR_NO_EMAIL_ADDRESS = 29002;
    ...
    */
    public function touch($post_id = 0)
    {
        ...
        mc4wp_ecommerce_update_settings([ 'last_updated' => time() ]);
    }
    ...
    */
    public function delete_order($order_id)
    {
        ...
        if ($this->hpos_enabled) {
            global $wpdb;
            $query = $wpdb->prepare("DELETE FROM {$wpdb->prefix}wc_orders_meta WHERE order_id = %d AND meta_key = %s", [ $order_id, self::META_KEY]);
            $wpdb->query($query);
        } else {
    ...
    */
    ...
    {
        $domain = str_ireplace([ 'https://', 'http://', '://' ], '', get_home_url());
    ...
    */
    ...
    private function delete_all_metadata()
    {
        ...
        if ($this->hpos_enabled) {
            $query = $wpdb->prepare("DELETE FROM {$wpdb->prefix}wc_orders_meta WHERE meta_key = %s", [ self::META_KEY]);
            $wpdb->query($query);
        }
    }
    ...
    public function ensure_connected_site()
    {
        ...
        try {
            ...
            $response = $client->post('/connected-sites', [
    ...
    */
    public function update_product($product)
    {
        ...
        return $this->is_product_tracked($product_id) ? $this->product_update($product, $product_data, $variants_data) : $this->product_add($product, $product_data, $variants_data);
    }
    ...
    * @throws MC4WP_API_Exception|Exception
    */
    private function product_add(WC_Product $product, array $product_data, $variants_data = [], $recurse = true)
    {
        $api = $this->get_api();
        $store_id = $this->store_id;
        $product_id = $this->transformer->product_id($product);
     ...
     * @throws MC4WP_API_Exception|Exception
     */
    private function product_update(WC_Product $product, array $product_data, $variants_data = [], $recurse = true)
    {
        $api = $this->get_api();
        $store_id = $this->store_id;
        $product_id = $this->transformer->product_id($product);
    ...
    public function is_order_tracked($order)
    {
        ...
        return !!$order->get_meta(self::META_KEY);
    }
    ...
    * @return bool
    */
    public function is_product_tracked($product_id)
    {
