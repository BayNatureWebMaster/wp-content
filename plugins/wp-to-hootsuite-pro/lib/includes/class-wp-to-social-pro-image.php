...
class WP_To_Social_Pro_Image {
	...
	public function get_social_media_image_size( $service, $format = false ) {
		 ...
		 * @param   bool|string $format        Status format (for example, 'story' or 'post' for Instagram).
		 */
		$image_size = apply_filters( $this->base->plugin->filter_name . '_get_social_media_image_size', $image_size, $service, $format );
	...
	private function get_social_media_image_sizes() {
		...
		// Defines the optimal image sizes for each social network.
		$image_sizes = array(
			'twitter'         => array( 1024, 512 ),
			'pinterest'       => array( 1200, 1200 ),
		 ...
		 */
		$image_sizes = apply_filters( $this->base->plugin->filter_name . '_get_social_media_image_sizes', $image_sizes );
