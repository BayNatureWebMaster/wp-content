...
if (!is_plugin_active('ultimate-social-media-plus/ultimate_social_media_icons.php')) {
	...
	<?php }
			...
			// function sfsi_premium_count_cronstarter_activation()
			// {
			// 	// sfsi_plus_write_log(wp_next_scheduled( 'sfsi_plus_sf_instagram_count_fetcher' ));
			// 	if (!wp_next_scheduled('sfsi_premium_save_calculated_count')) {
			// 		wp_schedule_event(time(), 'daily', 'sfsi_premium_save_calculated_count');
			// 	}
			// }
			// and make sure it's called whenever WordPress loads
			// add_action('wp', 'sfsi_premium_count_cronstarter_activation');
