...
class WP_To_Social_Pro_Hootsuite_API {
	...
	public function update_access_token() {
		...
		if ( ! $this->check_refresh_token_exists() ) {
			...
			return new WP_Error( 'missing_refresh_token', __( 'No refresh token exists', 'wp-to-social-pro' ) );
		}
		...
		if ( ! $body->success ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Error refreshing access token.  Hootsuite said: %s', 'wp-to-social-pro' ),
					$body->data
				)
			);
		}
	...
	private function get_formatted_service( $type ) {
		...
		switch ( $type ) {
			...
			case 'TWITTER':
				return __( 'Twitter', 'wp-to-social-pro' );

			case 'INSTAGRAM':
				return __( 'Instagram', 'wp-to-social-pro' );

			case 'FACEBOOKPAGE':
				return __( 'Facebook Page', 'wp-to-social-pro' );

			case 'FACEBOOK':
				return __( 'Facebook', 'wp-to-social-pro' );

			case 'LINKEDINCOMPANY':
				return __( 'LinkedIn Page', 'wp-to-social-pro' );

			case 'LINKEDIN':
				return __( 'LinkedIn', 'wp-to-social-pro' );

			case 'GOOGLEPLUSPAGE':
				return __( 'Google+ Page', 'wp-to-social-pro' );

			case 'PINTEREST':
				return __( 'Pinterest', 'wp-to-social-pro' );
		...
		if ( ! $file ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Attachment ID %1$s (%2$s) could not be found in the Media Library.', 'wp-to-social-pro' ),
					$id,
					$media_url
				)
			);
	...
	private function parse_response( $response, $http_code, $params, $url ) { // phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter
		...
		if ( is_null( $body ) ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Hootsuite API Error: %1$s returned a %2$s %3$s error. Please try again.', 'wp-to-social-pro' ),
					$url,
					$http_code,
					$response
				)
		...
		return new WP_Error(
			...
			sprintf(
				...
				__( 'Hootsuite API Error: #%1$s: %2$s', 'wp-to-social-pro' ),
				$http_code,
				implode( "\n", $message )
			)
		);
