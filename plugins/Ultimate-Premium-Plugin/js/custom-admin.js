...
function sfsi_plus_depened_sections(should_reload) {
    ...
    if (true !== should_reload) {
        /*  console.log(desktopIconElem,mobileIconElem); */
        ...
        arrIcons.forEach(function(iconName, index) {
            ...
            sfsi_plus_toogle_icon_for_order_icons_update(iconName);
            // sfsi_plus_toogle_mobile_icon_for_order_icons(iconName);
            // sfsi_plus_toogle_desktop_icon_for_mouserover_icons(iconName);
        });
    }
    ...
    SFSI.each(SFSI("ul.plus_icn_listing li.plus_custom input[element-ctype='sfsiplus-cusotm-icon']"), function(index, element) {
        ...
        console.log('sfsiplus-cusotm-icon11', customDElem);
        ...
        success: function(s) {
            if (s.res == 'success') {
                ...
                if (s.res == "jpgError") {
                    ...
                    sfsiplus_showErrorSuc("error", "Please upload an icon in .png format. You can convert images <a style='display: inline;text-decoration: underline;color:inherit !important' href='https://image.online-convert.com/convert-to-png'>here</a>.", 1)
                } else {
...
function sfsi_plus_update_step4() {
    ...
    var e = SFSI("input[name='sfsi_plus_display_counts']:checked").val(),
        ...
        wechat_From = 1 == SFSI("input[name='sfsi_plus_tumblr_countsFrom']").prop("disabled") ? "" : SFSI("input[name='sfsi_plus_tumblr_countsFrom']:checked").val(),
...
function sfsi_plus_make_popBox() {
    ...
    var headingTxt = escapeHtml(SFSI('textarea[name="sfsi_plus_popup_text"]').val().replace(new RegExp("\n", "g"), "<br>"));
...
function sfsi_premium_resize_icons_container() {
    ...
    sfsi_premium_inline_style = jQuery('.sfsi_premium_responsive_icon_preview .sfsi_premium_icons_container').attr('style');
    // remove_width 
    ...
    SFSI.ajax({
        ...
        success: function(s) {
            ...
            if (res.status) {
                ...
                alert('licence is not yet renewed. please follow the instructions on the notice to renew.');
            }
