...
function plus_UploadMouseOverIcons(){
	...
	sfsi_plus_icon_upload($iconName,$custom_imgurl,$iconPrefix,$customIconIndex,
...
function sfsi_premium_deleteIcons()
{
	...
	$response = array('status'=> false, 'message' => __( 'Failed to delete the icon', 'ultimate-social-media-plus' ) );
	...
	if(isset($icon_name) && !empty($icon_name) && isset($serviceType) && !empty($serviceType)){
		...
		switch ($serviceType) {
			...
			case 'mouseover':
				...
				if("custom" == $icon_name && $customIconIndex > -1 
...
function sfsi_plus_UploadIcons(){
	...
	if(!empty($custom_img_dirPath)):
		...
		if('success' == $response['res']){
			...
			$response = array('res'=>'success','img_path'=> $newImageUrl,'element'=>$total_uploads,'key'=>$key);
		}
...
function sfsi_plus_resize_gif($srcImageDirPath,$width,$height,$outImageDirPath){
	...
	if( isset($srcImageDirPath) && !empty($srcImageDirPath) && isset($width) && !empty($width) 
...
function sfsi_premium_deleteWebChatFollow()
{
	...
	if(is_array($option2)){
		...
		echo json_encode( array( 'res' => 'success', 'message' => __( 'removed the scan image for wechat', 'ultimate-social-media-plus' ) ) );
	} else {
		echo json_encode( array( 'res' => 'failed', 'message' => __( 'Couldn\'t load options', 'ultimate-social-media-plus' ) ) );
	}
	die;
}
