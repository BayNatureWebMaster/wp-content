...
// var_dump()
					$hoverdiv .= "<div class='icon2'><a href='https://twitter.com/intent/tweet?text=" . urlencode($twitter_text) . "' " . sfsi_plus_checkNewWindow($url) . "><img nopin=nopin class='sfsi_premium_wicon' src='" . $tweet_icon . "' alt='Tweet' title='Tweet' ></a></div>";
				}
			}
		...
		case "linkedin":
			...
			if (
				...
				if ($option2['sfsi_plus_linkedin_SharePage'] == "yes") {
					...
					$hoverdiv .= "<div class='icon2'><a href='https://www.linkedin.com/shareArticle?url=" . $current_url . "'  " . sfsi_plus_checkNewWindow($url) . " ><img class='sfsi_premium_wicon' nopin=nopin alt='Share' title='Share' src='" . $linkedin_share_icon . "' /></a></div>";
				}
		...
		case "pinterest":
			...
			if ($option2['sfsi_plus_pinterest_pingBlog'] == "yes" && $option2['sfsi_plus_pinterest_page'] == "yes") {
				...
				if ($option5['sfsi_premium_pinterest_sharing_texts_and_pics'] === "yes") {
					$hoverdiv .= "<div class='icon2'><a class ='sfsi_premium_pinterest_create'  onclick='sfsi_premium_pinterest_modal_images(\"" . $current_url . "\",\"" . ($description_escaped) . "\")'><img class='sfsi_premium_wicon' data-pin-nopin='true'  nopin=nopin alt='" . $alt_text . "' title='" . $alt_text . "' src='" . $pinterest_save . "' /></a></div>";
				} else {
		...
		case "yummly":
			...
			$sharingUrl = "https://www.yummly.com/urb/verify?url=" . trailingslashit($current_url) . "&title=" . $post_title . "&yumtype=button";
		...
		case "reddit":
			...
			if ($option2['sfsi_plus_reddit_url_type'] == 'share') {
				$url = 'https://reddit.com/submit?url=' . $current_url . '&title=' . get_the_title();
		...
		case "fbmessenger":
			...
			if (wp_is_mobile()) {
				$url = "fb-messenger://share/?link=" . trailingslashit($current_url);
			} else {
				...
				$url = "https://www.facebook.com/dialog/send?app_id=" . $app_id . "&display=popup&link=" . trailingslashit($current_url) . "&redirect_uri=" . trailingslashit($current_url);
			}
		...
		case "gab":
			...
			$post_title = $socialObj->sfsi_get_the_title();
			$url = "https://gab.com/compose?url=" . trailingslashit( $current_url). "&text=".$post_title;
			$toolClass = "rss_tool_bdr";
			$hoverdiv = '';
			$arsfsiplus_row_class = "bot_rss_arow";
		...
		case "mix":
			...
			$url = "https://mix.com/mixit?url=" . trailingslashit($current_url);
		...
		case "ok":
			...
			$url = "https://connect.ok.ru/offer?url=" . trailingslashit($current_url);
		...
		case "telegram":
			...
			$url = "https://telegram.me/share/url?url=" . trailingslashit($current_url);

			if ("yes" == $option2['sfsi_plus_telegramShare_option'] && "yes" == $option2['sfsi_plus_telegramMessage_option']) {
				$hoverSHow = 1;

				if (
					isset($option2['sfsi_plus_telegram_message']) && !empty($option2['sfsi_plus_telegram_message'])
					&&
					isset($option2['sfsi_plus_telegram_username']) && !empty($option2['sfsi_plus_telegram_username'])
		...
		case "xing":
			...
			$url = "https://www.xing.com/app/user?op=share&url=" . trailingslashit($current_url);
		...
		case "mastodon":
			...
			$mastodon_share_btn_html = "
				...
				<mastodon-share-button
					...
					icon_url=''
					share_button_text=''
					style='visibility:none; position: absolute; top: 0; left: 0; z-index: 0;'
...
function sfsi_premium_sticky_bar_icons($content, $option8, $server_side = false, $option5 = null) {
	...
	<?php
		if ($sfsi_plus_sticky_icons['settings']['desktop'] == "yes" && !wp_is_mobile()) {
			$icons .= "\t<div class='sfsi_premium_desktop_display sfsi_premium_sticky_icons_container sfsi_premium_sticky_" . strtolower($sfsi_plus_sticky_icons['settings']['desktop_placement']) . "_button_container sfsi_premium_sticky_" . strtolower($sfsi_plus_sticky_icons['settings']['desktop_placement_direction']) .  "  ' style='text-align:center;' >";
	...
	<div class="sfsi_premium_sticky_icons_count sfsi_premium_sticky_count_container" style='text-align:center;display:<?php echo $sfsi_plus_sticky_icons['settings']['counts'] == "yes" ? "block" : "none"; ?> ;background-color:<?php echo $sfsi_plus_sticky_icons['settings']['bg_color']; ?>;color:<?php echo $sfsi_plus_sticky_icons['settings']['color']; ?>;'>
		...
		<span class="sfsi_premium_count_text" style="color:<?php echo $sfsi_plus_sticky_icons['settings']['color']; ?>;">
				...
				foreach ($sfsi_plus_sticky_icons['default_icons'] as $icon => $icon_config) {
					...
					switch ($icon) {
						...
						case "Twitter":
							...
							$share_url = "https://twitter.com/intent/tweet?text=" . $twitter_text . "&url=";
							break;
						case "Follow":
							if (isset($option2['sfsi_plus_email_icons_functions']) && $option2['sfsi_plus_email_icons_functions'] == 'sf') {
						...
						case "pinterest":
							$share_url = 'https://www.pinterest.com/pin/create/link/?url=' . $current_url . '&media=' . $socialObj->sfsi_pinit_image($option5) . '&description=' . $socialObj->sfsi_pinit_description($option5);
							$is_pinterest = true;
							break;

						case "Linkedin":
							$share_url = "http://www.linkedin.com/shareArticle?mini=true&url=" . $current_url;
							break;
						case "Whatsapp":
							$share_url = wp_is_mobile() ? 'https://api.whatsapp.com/send?text=' . $current_url : 'https://web.whatsapp.com/send?text=' . $current_url;
							break;
						case "vk":
							$share_url = "http://vk.com/share.php?url=" . trailingslashit($current_url);
							break;
						case "Odnoklassniki":
							$share_url = "https://connect.ok.ru/offer?url=" . trailingslashit($current_url);
							break;
						case "Telegram":
							$share_url = "https://telegram.me/share/url?url=" . trailingslashit($current_url);
							break;
						case "Weibo":
							$share_url = "http://service.weibo.com/share/share.php?url=" . trailingslashit($current_url);
							break;
						case "QQ2":
							$share_url = "http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=" . trailingslashit($current_url);
							break;
						case "xing":
							$share_url = "https://www.xing.com/app/user?op=share&url=" . trailingslashit($current_url);
							break;
						case "mastodon":
							$share_url = "https://mastodon.social/share?text=" . trailingslashit($current_url);
							break;
					}
					if (false == $is_pinterest || (true == $is_pinterest && $icon_config['url'] !== "")) {
						...
						$icons .= "<div class='sfsi_premium_sticky_icon_item_container sfsi_premium_responsive_icon_" . strtolower($icon) . "_container' >";
				...
				foreach ($sfsi_plus_sticky_icons_custom_icons as $icon => $icon_config) {
					...
					if ( ( isset( $icon_config['active'] ) && 'yes' === $icon_config['active'] ) && ( isset( $icon_config['added'] ) && 'yes' === $icon_config['added'] ) ) {
						...
						$icons .= "<a " . sfsi_plus_checkNewWindow() . " href='" . ($icon_config['url'] == "" ? "" : do_shortcode($icon_config['url'])) . "'>";
							$icons .= "<div class='sfsi_premium_sticky_icon_item_container sfsi_premium_sticky_custom_icon sfsi_premium_sticky_icon_" . strtolower($icon) . "_container' style='background-color:" . (isset($icon_config['bg-color']) ? $icon_config['bg-color'] : '#73d17c') . "'>";
								if ( isset( $icon_config['icon'] ) && !empty( $icon_config['icon'] ) ) {
									$icons .= "<img alt='' style='max-height: 25px' src='" . $icon_config['icon'] . "'>";
								}
							$icons .= "</div>";
						$icons .= "</a>";
					}
