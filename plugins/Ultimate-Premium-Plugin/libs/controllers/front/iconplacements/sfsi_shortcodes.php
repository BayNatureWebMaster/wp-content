		...
		if (sfsi_plus_shall_show_icons('rect_icon_shortcode')) {
			...
			if ( $postid ) {
				...
				if (isset($sfsi_section8['sfsi_plus_recttwtr']) && ('yes' === $sfsi_section8['sfsi_plus_recttwtr'])) {
					...
					$icons .= "<div class='sf_twiter' style='display: inline-flex;vertical-align: middle;width: auto;'><a href='https://twitter.com/intent/tweet?text=" . urlencode($twitter_text) . "' " . sfsi_plus_checkNewWindow() . " style='display:inline-block' ><img nopin=nopin class='sfsi_premium_wicon' src='" . $tweet_icon . "' alt='Tweet' title='Tweet' ></a>" . $count_html . "</div>";
				}
				if ($show_count) {
