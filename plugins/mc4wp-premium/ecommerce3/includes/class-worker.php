...
class MC4WP_Ecommerce_Worker
{
    ...
    {
        add_action('mc4wp_ecommerce_process_queue', [ $this, 'work' ]);
    }
    ...
    public function work()
    {
        ...
        while (($job = $this->queue->get()) && $job instanceof MC4WP_Queue_Job) {
            ...
            try {
                $success = call_user_func_array([ $this, $job->data['method'] ], $job->data['args']);
            } catch (Error $e) {
    ...
    public function delete_order($id)
    {
        ...
        if (! $this->ecommerce->is_order_tracked($id)) {
            return false;
        }
     ...
     * @return bool
     */
    public function update_cart($cart_id, $customer, $cart_contents = [])
    {
    ...
    public function update_subscriber_email($old_email_address, $new_email_address)
    {
        ...
        try {
            ...
            $api->update_list_member($list_id, $old_email_address, [ 'email_address' => $new_email_address ]);
        } catch (MC4WP_API_Resource_Not_Found_Exception $e) {
