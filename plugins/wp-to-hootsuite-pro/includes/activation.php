 ...
 */
function wp_to_hootsuite_pro_activate( $network_wide ) {

	// If the Free version of the Plugin is activated, deactivate it now.
	if ( is_plugin_active( 'wp-to-hootsuite/wp-to-hootsuite.php' ) ) {
		deactivate_plugins( 'wp-to-hootsuite/wp-to-hootsuite.php' );
	}
	...
	if ( ! is_multisite() || ! $network_wide ) {
		...
		$wp_to_hootsuite_pro->get_class( 'install' )->install();
	} else {
 ...
 */
function wp_to_hootsuite_pro_activate_new_site( $site_or_blog_id ) {
