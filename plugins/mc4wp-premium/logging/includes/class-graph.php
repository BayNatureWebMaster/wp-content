    ...
    public function init()
    {
        ...
        switch ($this->range) {
            ...
            case 'this_quarter':
                ...
                $start_date              = new DateTime(gmdate(sprintf('Y-%02d-01 00:00:00', $month)), $timezone);
            ...
            case 'last_quarter':
                $month = floor((int) $now->format('m') / 3) * 3 + 1 - 3;
                if ($month < 0) {
                    $month += 12;
                }
                $start_date              = new DateTime(gmdate(sprintf('Y-%02d-01 00:00:00', $month)), $timezone);
            ...
            case 'custom':
                $start_date              = new DateTime(implode('-', [
                        $this->config['start_year'],
                        $this->config['start_month'],
                        $this->config['start_day']
                    ]) . ' 00:00:00', $timezone);
                $end_date                = new DateTime(implode('-', [
                        $this->config['end_year'],
                        $this->config['end_month'],
                        $this->config['end_day']
                    ]) . ' 23:59:59', $timezone);
